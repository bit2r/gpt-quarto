...
따라서, 사람뇌에 인식할 수 있는 시각적인 정보로 데이터를 구성해야만 다양한 종류의 모니터를 통해 효율적이고 효과적으로 정보가 전달될 수 있다. [^theme_colors-1]
\index{RGB}
...
![시각적 인지 메커니즘](images/color-viz-mechanism.jpg){#fig-viz-mechanisms}
...
```{r}
#| echo: false
#| label: tbl-rgb-intensity
#| tbl-cap: 무책색(흰색, 회색, 흑백) 색상표
...
![RGB 색상](data/color_rgb.pdf){#fig-color-rgb height="300px" width="300px"}
:::
...
![CMYK 색상](data/color_cmyk.pdf){#fig-color-cmyk height="300px" width="300px"}
:::
:::
...
```{r}
#| out-width: 100%
#| label: fig-viridis-penguins
#| fig-cap: 팔머 펭귄 데이터에 viridis 팔렛트 적용
# 패키지 로드
library(ggplot2)
library(viridis)
library(palmerpenguins)
